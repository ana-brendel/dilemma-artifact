...
Theorem goal80_by_rev_append : forall (x : Lst) (y : Lst), eq (rev (append (rev x) (rev y))) (append y x).
Proof.
  induction x.
  - intros. simpl. rewrite rev_rev. rewrite append_nil. reflexivity.
  - intros. simpl. 
  dilemma. Admitted.
  (* rewrite rev_append. simpl. rewrite (eq_refl : cons n nil = rev (cons n nil)). rewrite IHx. 
