...
Lemma even_odd_plus: forall m n, even (plus m n) = negb (even (plus m (succ n))).
Proof.
induction m.
...
Theorem goal16_by_even_odd_plus : forall (x : Nat), eq (even (plus x x)) true.
Proof.
  induction x.
  - simpl. 
  dilemma. Admitted.

  (* rewrite <- even_odd_plus. assumption.
  - reflexivity.
  Qed. *)
