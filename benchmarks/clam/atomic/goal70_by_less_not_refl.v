...
Theorem goal70_by_less_not_refl : forall (x : Nat) (y : Lst), eq (count x (insort x y)) (succ (count x y)).
Proof.
  intros.
  induction y.
  ...
  - simpl. destruct (Nat_beq x n) eqn:?.
    + destruct (less x n) eqn:?.
      * apply Nat_beq_eq in Heqb. rewrite Heqb in Heqb0. 
      dilemma. Admitted.
      (* rewrite less_not_refl in Heqb0. discriminate.
