...
Lemma goal33_by_mult_commut: forall (x y : Nat), qfac x y = mult (qfac x (succ zero)) y.
Proof.
  intro.
  induction x.
  - intros. simpl. rewrite IHx. rewrite (IHx x). rewrite mult_assoc. 
  dilemma. Admitted.
  (* rewrite (mult_commut y x). reflexivity.
  - reflexivity.
Qed. *)
